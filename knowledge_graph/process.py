...
def run(neo4j_url):
    ...
    with driver.session() as session:
        session.run("CREATE CONSTRAINT ON (n:Conceptnet) ASSERT n.uri IS UNIQUE")
        session.run("CREATE INDEX ON :Conceptnet(name)")
        session.run("CREATE INDEX ON :Conceptnet(language)")
